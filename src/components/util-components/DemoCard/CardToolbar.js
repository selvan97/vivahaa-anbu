	...
	return (
		<div css={codeBoxActionCss(token)}>
			<span css={codeBoxIconCss(token, copied && copyTooltipVisible)}>
				<Tooltip title={copied ? 'Copied' : 'Copy code'} open={copyTooltipVisible} onOpenChange={handleCopyTooltipVisibleChange}>
			...
			<span css={codeBoxIconCss(token)} onClick={expand}>
				<Tooltip title={isExpand ? 'Hide code' : 'Show code'}>{isExpand ? <UpSquareOutlined /> : <DownSquareOutlined />}</Tooltip>
			</span>
		</div>
	);
};
