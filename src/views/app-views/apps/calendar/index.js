...
const badgeColors = ['pink', 'red', 'yellow', 'orange', 'cyan', 'green', 'blue', 'purple', 'geekblue', 'magenta', 'volcano', 'gold', 'lime'];
...
const EventModal = ({ open, addEvent, cancel }) => {
	...
	return (
		<Modal title="New Event" open={open} footer={null} destroyOnClose={true} onCancel={cancel}>
			<Form form={form} layout="vertical" name="new-event" preserve={false} onFinish={onSubmit}>
		...
		<Card className="calendar mb-0">
			<Row>
				...
				<Col xs={24} sm={24} md={15} lg={18}>
					<Calendar onSelect={(val) => onSelect(val)} dateCellRender={cellRender} />
				</Col>
			</Row>
			<EventModal open={modalVisible} addEvent={onAddEvent} cancel={onAddEventCancel} />
		</Card>
	);
};
