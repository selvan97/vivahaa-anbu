...
const Demo = () => {
	...
	return (
		<>
			<Flex alignItems={alignItems} justifyContent={justifyContent} flexDirection={flexDirection}>
				<>
					{arr.map((elm, index) => (
						<div key={elm} className={`bg-${elm} m-2`} style={{ width: 40, height: index === 1 ? 25 : 40, borderRadius: 4 }}></div>
					))}
				</>
			</Flex>
			<div className="mt-4">
