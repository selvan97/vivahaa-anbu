			...
			<div className="mt-4">
				...
				<ol>
					<li>
						<strong>Setup</strong>
						<p class="mt-1">
							Visit <code>src/mock/index.js</code>, and here is the overall configuration of mirage
						</p>
	...
	return createServer({
		...
		routes() {
			this.get('api/getUsers', schema => schema.db.usersData)
				...
				<p>
					And now you will get the mock data as response when you make request to <code>api/getUsers</code>
				</p>
			</div>
			<div className="mt-4">
				<h4>Disable mock api</h4>
				<p>
					Mock api was enable by default in our starter-kit, you can turn it off by remove code below in <code>App.js</code>
				</p>
				<SyntaxHighlighter className="hl-code" language="javascript" style={atomDark}>
					{`// Remove this block
