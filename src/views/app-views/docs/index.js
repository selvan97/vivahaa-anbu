...
const DocsMenu = () => {
	...
	return (
		<div className="w-100">
			<Menu defaultSelectedKeys={`${url}/introduction`} mode="inline" style={{ borderInlineEnd: 0 }} selectedKeys={[currentpath]} items={menuItems} />
		</div>
	);
};
					...
					<div className="container-fluid">
						<Suspense fallback={<Loading />}>
							<Routes>
								{menuList.map((elm) =>
									elm.sub.length === 0 ? (
										<Route key={elm.key} path={`${elm.key}`} element={<ElementRoute component={lazy(() => import(`./components/${elm.name.replace(/\s/g, '')}`))} />} />
									) : (
										elm.sub.map((item) => (
											<Route key={item.key} path={`${item.key}`} element={<ElementRoute component={lazy(() => import(`./components/${item.name.replace(/\s/g, '')}`))} />} />
										))
									),
								)}
