...
export class MapChartWithTexture extends Component {
	render() {
		return (
			...
			>
				<PatternLines id="lines" height={6} width={6} stroke="#776865" strokeWidth={1} background="#F6F0E9" orientation={['diagonal']} />
				<Sphere stroke="#DDD" />
				...
				<Geographies geography={geoUrl} stroke="#FFF" strokeWidth={0.5}>
					{({ geographies }) =>
						geographies.map((geo) => {
							...
							return <Geography key={geo.rsmKey} geography={geo} fill={isHighlighted ? "url('#lines')" : '#F6F0E9'} onClick={() => console.log(geo.properties.ISO_A3)} />;
						})
					}
				</Geographies>
