...
class IconDisplay extends Component {
	...
	renderCategories() {
		...
		return Object.keys(categories)
			...
			.map(({ category, icons }) => <Category key={category} title={category} theme={theme} icons={icons} newIcons={IconDisplay.newIconNames} />);
	}
		...
		return (
			<>
				<div className="d-md-flex">
					<Radio.Group value={this.state.theme} onChange={this.handleChangeTheme} buttonStyle="solid">
					...
					<div className="mt-3 mt-md-0 ml-md-3 d-inline-flex flex-fill">
						<Input.Search placeholder="Search icon here" allowClear onChange={(e) => this.handleSearchIcon(e.currentTarget.value)} autoFocus />
					</div>
				</div>
