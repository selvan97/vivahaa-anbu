	...
	return (
		<Form name="edit-card-ref" layout="vertical" onFinish={submitUpdate} initialValues={initialValues}>
			...
			<Form.Item label="Assigned to" name="members" className="blockform-col col-3">
				<Select filterOption={false} tagRender={memberTagRender} mode="tags" removeIcon={null} placeholder="None" className="board-card-modal select">
					{memberIds.map((elm) => (
			...
			<Form.Item label="Labels" name="labels" className="blockform-col col-3">
				<Select filterOption={false} tagRender={labelTagRender} mode="tags" removeIcon={null} placeholder="None" className="board-card-modal select">
					{labels.map((elm) => (
					...
					<div className="mb-3 d-flex">
						...
						<Card className="ml-2 w-100">
							<Input
								...
								suffix={
									<div onClick={submitComment} className="cursor-pointer font-weight-semibold text-primary">
										Send
									</div>
								}
							/>
					...
					{(() => {
						switch (modalMode) {
							...
							case modalModeTypes(1):
								return <UpdateCardForm cardData={cardData} listId={listId} onSubmit={(values) => submit(values, modalModeTypes(1))} />;
							case modalModeTypes(2):
