			...
			{props.contents?.map((item, index) => (
				<Draggable key={item.id} draggableId={item.id} index={index}>
					{(dragProvided, dragSnapshot) => (
						<div className="mb-3" key={item.id} ref={dragProvided.innerRef} {...dragProvided.draggableProps} {...dragProvided.dragHandleProps}>
							<Card hoverable className="board-card" cover={null} onClick={() => props.cardData(item, props.listId)}>
...
export default function BoardCard(props) {
	const { ignoreContainerClipping, internalScroll, scrollContainerStyle, isDropDisabled, isCombineEnabled, listId = 'LIST', listType, style, contents, useClone, cardData } = props;
	return (
		<>
			<Droppable
				droppableId={listId}
					...
					<Scrollbars style={style} className="board-wrapper flex-auto" autoHide {...dropProvided.droppableProps}>
						{internalScroll ? (
							<div className="board-scrollContainer" style={scrollContainerStyle}>
								<InnerList contents={contents} listId={listId} cardData={cardData} dropProvided={dropProvided} />
							</div>
						) : (
							<InnerList contents={contents} listId={listId} cardData={cardData} dropProvided={dropProvided} />
						)}
